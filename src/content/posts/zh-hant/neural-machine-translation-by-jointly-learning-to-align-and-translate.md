---
title: 論文共讀：《Neural Machine Translation by Jointly Learning to Align and Translate》
date: 2026-01-11
category: "Paper Reading"
description: 注意力機制的起源，附 Rust 複現程式碼
tags: [paper-reading, attention, AI, LLM, rust]
pinned: false
---

2014 年 9 月 1 日，三個人在 arXiv（一個學術論文預印本網站，論文不用等期刊審稿就能直接發布）上傳了一篇論文：<a href="/papers/Neural%20Machine%20Translation%20by%20Jointly%20Learning%20to%20Align%20and%20Translate.pdf" target="_blank">《Neural Machine Translation by Jointly Learning to Align and Translate》</a>（通過聯合學習對齊與翻譯實現神經機器翻譯）。
...
三年後，Google 的八個人把這個想法推到了極致，寫出了[《Attention Is All You Need》](/zh-hant/posts/attention-is-all-you-need/)。所以如果你想理解 Transformer，這篇論文是最重要的前史之一。
...
fn bahdanau_attention(
    decoder_state: &Tensor,  // 解碼器當前狀態 s_{i-1}
    encoder_outputs: &Tensor, // 編碼器所有位置的隱藏狀態 [h_1, ..., h_T]
    w_a: &Tensor,            // 權重矩陣 W_a
    u_a: &Tensor,            // 權重矩陣 U_a
    v_a: &Tensor,            // 向量 v_a
) -> (Tensor, Tensor) {
...
struct BidirectionalRNN {
    forward_rnn: RNN,   // 從左往右讀
    backward_rnn: RNN,  // 從右往左讀
}

impl BidirectionalRNN {
    fn forward(&self, input: &[Tensor]) -> Vec<Tensor> {
        let fwd = self.forward_rnn.forward(input);          // 正向隱藏狀態
        let bwd = self.backward_rnn.forward_reversed(input); // 反向隱藏狀態
        // 把正向和反向的隱藏狀態拼起來
...
impl AttentionDecoder {
    fn decode_step(
        &self,
        prev_word: &Tensor,       // 上一步生成的詞
        prev_state: &Tensor,      // 上一步的隱藏狀態
        encoder_outputs: &Tensor, // 編碼器所有位置的隱藏狀態
    ) -> (Tensor, Tensor) {
        // 注意力：決定看源句子的哪些部分
        let (context, _weights) = self.attention.forward(prev_state, encoder_outputs);
        // RNN 更新狀態：融合上下文、上一個詞和上一步狀態
...
第三，用 Rust 複現這篇論文的注意力機制時，你會發現它的計算流程比 Transformer 的 Scaled Dot-Product Attention 複雜不少。加性注意力需要額外的權重矩陣 W_a、U_a、v_a，而點積注意力只需要 Q 和 K 直接相乘再縮放。從「加法」到「乘法」，看似一小步，實際上大幅簡化了計算，也更適合用矩陣運算高效實現。
