...
其中權重 `α_{i -> l}` 來自一層 softmax。你可以先把 softmax 理解成「把一組分數壓成一組權重，而且所有權重加起來等於 1」，這樣模型才能明確表達「更該看誰、少看誰」：
...
2026 年春，Kimi 團隊的工作已經說明：當既有 Scaling Laws 開始顯出逼近瓶頸的跡象時，LLM 的結構創新將持續湧現。
