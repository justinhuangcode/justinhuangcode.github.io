---
title: 论文共读：《Attention Is All You Need》
date: 2026-01-06
category: "Paper Reading"
description: 分享我对 Transformer 论文的理解，附 Rust 复现代码
...
这篇文章是我读完论文后的理解，附带 Rust 复现的核心代码。不是翻译，不是摘要。没有技术背景也能读下去。
...
fn scaled_dot_product_attention(
    query: &Tensor,   // 每个词「在找什么」
    key: &Tensor,     // 每个词「能提供什么」
    value: &Tensor,   // 每个词「实际携带的内容」
) -> Tensor {
    let d_k = key.size(-1) as f64;       // 向量长度
    let scores = query.matmul(           // 点积：算匹配分数
        &key.transpose(-2, -1)
    ) / d_k.sqrt();                       // 缩放，防止分数过大
    let weights = scores.softmax(-1);     // 转成概率（所有概率加起来等于 1）
    weights.matmul(value)                 // 用概率加权，提取信息
}
```
...
struct MultiHeadAttention {
    heads: Vec<AttentionHead>,    // 8 个注意力头，各自独立运算
    output_proj: Linear,          // W^O：汇总所有头的结果
}
...
fn positional_encoding(seq_len: usize, d_model: usize) -> Tensor {
    ...
    for pos in 0..seq_len {                          // 遍历每个位置
        for i in (0..d_model).step_by(2) {           // 每次处理一对维度（偶数 + 奇数）
            let angle = pos as f64               // 位置编号
                / (10000_f64).powf(i as f64 / d_model as f64);  // 除以缩放因子
            encoding[[pos, i]] = angle.sin();        // 偶数维度用 sin
            if i + 1 < d_model {
                encoding[[pos, i + 1]] = angle.cos();// 奇数维度用 cos
            }
        }
    }
    encoding
...
struct Transformer {
    encoder_layers: Vec<EncoderLayer>,  // 6 层编码器
    decoder_layers: Vec<DecoderLayer>,  // 6 层解码器
}

struct EncoderLayer {
    self_attention: MultiHeadAttention, // 自注意力：句子内部词与词的关系
    feed_forward: FeedForward,          // 前馈网络：对每个位置独立变换
    norm1: LayerNorm,                   // 第一个归一化层
    norm2: LayerNorm,                   // 第二个归一化层
    dropout: f64, // 0.1               // 训练时随机关掉 10% 的通路，防止模型「死记硬背」训练数据
}

struct DecoderLayer {
    masked_self_attention: MultiHeadAttention, // 带遮罩的自注意力：只能看已生成的词
    cross_attention: MultiHeadAttention,       // 交叉注意力：回头看编码器的输出
    feed_forward: FeedForward,                 // 前馈网络
    norm1: LayerNorm,
    norm2: LayerNorm,
    norm3: LayerNorm,                          // 多一层归一化（对应多出的交叉注意力）
    dropout: f64, // 0.1
}
```
...
第二，用 Rust 复现的过程让我更深地理解了每一个设计决策。当你自己写出 Scaled Dot-Product Attention，你会切实感受到那个 √d_k 的缩放有多重要。当你实现 masking，你会理解自回归生成的约束从何而来。论文读十遍，不如自己写一遍。
