---
title: 论文共读：《Language Models are Few-Shot Learners》
date: 2026-02-11
category: "Paper Reading"
description: 更大的模型，更善于从上下文中诱发能力，附 Rust 复现代码
tags: [paper-reading, gpt-3, AI, LLM, rust]
pinned: false
---

2020 年 5 月 28 日，OpenAI 在 arXiv（一个学术论文预印本网站，论文不用等期刊审稿就能直接发布）上传了一篇 75 页的论文：<a href="/papers/Language%20Models%20are%20Few-Shot%20Learners.pdf" target="_blank">《Language Models are Few-Shot Learners》</a>（语言模型是少样本学习者）。
...
enum EvalSetting {
    ...
    /// 任务描述 + 多个示例（通常 10-100 个）
    FewShot {
        instruction: String,
        examples: Vec<(String, String)>,  // 尽可能多地塞进上下文窗口
        prompt: String,
    },
}
...
struct GPT3Config {
    n_params: u64,       // 参数总数
    n_layers: usize,     // Transformer 层数
    d_model: usize,      // 隐藏维度
    n_heads: usize,      // 注意力头数
    d_head: usize,       // 每个头的维度（d_model / n_heads）
    d_ff: usize,         // 前馈网络维度（4 * d_model）
    n_ctx: usize,        // 上下文窗口长度
}
...
fn in_context_learning(
    ...
    // 第二步：分词
    let tokens = tokenize(&prompt);  // BPE 分词，词表大小约 50,000
