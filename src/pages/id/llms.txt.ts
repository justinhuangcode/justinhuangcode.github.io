...
export async function GET() {
  const posts = await getPostsByLocale('id');
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/id/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /id/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/id/posts/${slug}/)${desc}`;
    }),
    '',
  ];
