...
export async function GET() {
  const posts = await getPostsByLocale('ru');
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/ru/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /ru/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/ru/posts/${slug}/)${desc}`;
    }),
    '',
  ];
