...
export async function GET() {
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/it/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /it/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/it/posts/${slug}/)${desc}`;
    }),
    '',
  ];
