...
export async function GET() {
  const posts = await getPostsByLocale('zh-hans');
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/zh-hans/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /zh-hans/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/zh-hans/posts/${slug}/)${desc}`;
    }),
    '',
  ];
