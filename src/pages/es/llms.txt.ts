...
export async function GET() {
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/es/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /es/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/es/posts/${slug}/)${desc}`;
    }),
    '',
  ];
