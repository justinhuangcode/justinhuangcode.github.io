...
export async function GET() {
  const posts = await getPostsByLocale('zh-hant');
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/zh-hant/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /zh-hant/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/zh-hant/posts/${slug}/)${desc}`;
    }),
    '',
  ];
