...
export async function GET() {
  ...
  const lines = [
    ...
    `- Site: ${site}`,
    `- RSS: ${site}/rss.xml`,
    `- Full content: ${site}/fr/llms-full.txt`,
    `- Individual posts: append .md to any post URL (e.g. /fr/posts/hello-world.md)`,
    ...
    ...posts.map((post) => {
      ...
      return `- [${post.data.title}](${site}/fr/posts/${slug}/)${desc}`;
    }),
    '',
  ];
